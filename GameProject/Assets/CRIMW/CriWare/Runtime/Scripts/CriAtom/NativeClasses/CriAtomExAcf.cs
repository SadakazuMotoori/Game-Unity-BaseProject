...
public class CriAtomExAcf
{
    ...
    {
		public IntPtr namePointer;				/**< 名前の文字列ポインタ */
        ...
        public float pan3dWideness;             /**< Pan3D スプレッド(旧形式) */
    ...
    public struct AcfInfo
    {
        ...
        [MarshalAs(UnmanagedType.LPStr)]
        public string userData;                         /**< ユーザーデータ */
        ...
        public AcfInfo(byte[] data, int startIndex)
        {
            ...
            else
            {
                ...
                this.characterEncoding = (CharacterEncoding)Enum.ToObject(typeof(CharacterEncoding), BitConverter.ToUInt32(data, startIndex + 24));
                this.numDspSettings = BitConverter.ToInt32(data, startIndex + 28);
                this.numCategories = BitConverter.ToInt32(data, startIndex + 32);
                this.numCategoriesPerPlayback = BitConverter.ToInt32(data, startIndex + 36);
                this.numReacts = BitConverter.ToInt32(data, startIndex + 40);
                this.numAisacControls = BitConverter.ToInt32(data, startIndex + 44);
                this.numGlobalAisacs = BitConverter.ToInt32(data, startIndex + 48);
                this.numGameVariables = BitConverter.ToInt32(data, startIndex + 52);
                this.maxBusesOfDspBusSettings = BitConverter.ToInt32(data, startIndex + 56);
                this.numBuses = BitConverter.ToInt32(data, startIndex + 60);
                this.numVoiceLimitGroups = BitConverter.ToInt32(data, startIndex + 64);
                this.numOutputPorts = BitConverter.ToInt32(data, startIndex + 68);
            }
        }
    }
	 ...
	 * 以下の関数に引数として渡します。<br/>
	 * - <see cref='GetSelectorInfoByIndex'/><br/>
	 * - <see cref='GetSelectorInfoByName'/><br/></para>
	 * </remarks>
	 * <seealso cref='CriWare.CriAtomExAcf.GetSelectorInfoByIndex'/>
	 * <seealso cref='CriWare.CriAtomExAcf.GetSelectorInfoByName'/>
	 */
    [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi)]
	 ...
	 * </remarks>
	 * <seealso cref='CriWare.CriAtomExAcf.GetSelectorLabelInfo'/>
	 */
    [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi)]
    ...
    /**
     * <summary>オンメモリACBデータを用いたインゲームプレビューに必要な追加のワーク領域サイズの取得</summary>
     * <returns>追加のワーク領域サイズ</returns>
     * <remarks>
     * <para header='説明'>オンメモリACBデータを用いたインゲームプレビューに必要な追加のワーク領域サイズを取得します。<br/>
	 * オンメモリACBデータを用いてインゲームプレビューを開始するためには、
	 * <see cref='CriWare.CriAtomExAcb.LoadAcbData(IntPtr, int, CriWare.CriFsBinder, string)'/> に渡す
	 * ACBデータ領域の末尾に、本関数で取得したサイズの空きワーク領域を追加しておく必要があります。<br/></para>
     * <para header='注意'>ACBデータを読み込むメモリ領域を確保する前に本関数を呼び出し、必要な追加のワーク領域を確保してください。<br/>
	 * <see cref='CriWare.CriAtomExAcb.LoadAcbData(IntPtr, int, CriWare.CriFsBinder, string)'/> の
	 * dataSize 引数には、ACBデータサイズと空きワーク領域サイズの合計値を指定してください。<br/></para>
     * <para header='備考'>追加のワーク領域のサイズはライブラリに登録されたACFデータから取得します。<br/>
	 * ACFデータが登録されていない場合、固定値を返します。<br/></para>
     * </remarks>
     * <seealso cref='CriWare.CriAtomExAcb.LoadAcbData(IntPtr, int, CriWare.CriFsBinder, string)'/>
     */
    public static int GetExtraWorkSizeForLoadAcbData()
    {
#if CRIWARE_SUPPORTS_CRIATOMEXMONITOR
        return criAtomExAcf_GetExtraWorkSizeForLoadAcbData();
#else
        return 0;
#endif
    }
...
#if CRIWARE_SUPPORTS_CRIATOMEXMONITOR
    [DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
    private static extern int criAtomExAcf_GetExtraWorkSizeForLoadAcbData();
#endif //CRIWARE_SUPPORTS_CRIATOMEXMONITOR
#else
    ...
    private static bool criAtomExAcf_GetDspBusInformation(ushort index, out AcfDspBusInfo info) { info = new AcfDspBusInfo(); return false; }
...
#if CRIWARE_SUPPORTS_CRIATOMEXMONITOR
    private static int criAtomExAcf_GetExtraWorkSizeForLoadAcbData() { return 0; }
#endif //CRIWARE_SUPPORTS_CRIATOMEXMONITOR
#endif
    #endregion
}
