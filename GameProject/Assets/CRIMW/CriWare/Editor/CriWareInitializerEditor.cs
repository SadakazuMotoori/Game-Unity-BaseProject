...
public class CriWareInitializerEditor : UnityEditor.Editor
{
	...
	static private string[] asrOutputModes = {"Default", "2ch", "4ch", "6ch", "8ch", "12ch"};
	...
	public override void OnInspectorGUI()
	{
		...
		GUI.changed = false;
		{
			...
			EditorGUI.indentLevel += 1;
			{
				...
				GenPositiveFloatField("Max Pitch", "[cent]", "", ref initializer.atomConfig.maxPitch, 0.0f, 12000f);
				...
				initializer.atomConfig.speakerMapping = (CriAtom.SpeakerMapping)GenEnumField("ASR Rendering Mode", "", initializer.atomConfig.speakerMapping);
				...
				selected_output_mode = EditorGUILayout.Popup("ASR Output Channels", selected_output_mode, asrOutputModes);
				...
				GenToggleField("Disable Aisac Graph Cruve Type", "Disables curve types in AISAC graphs and automation, treating them as linear.", ref initializer.atomConfig.disableAisacGraphCurveType);
				...
				if (showAtomWebGLConfig) {
					...
					GenToggleField("Use WebAudio Mixer as Default", "Use WebAudio Mixer for DSP Bus Settings APIs. Check this option when using AAC codec. Some DSP effects are unsupported and ignored.",
						ref initializer.atomConfig.webglUseWebAudioMixerAsDefault);
					GenIntFieldWithUnit("Buffering Time", "[msec]", "Sound buffering time in msec.", ref initializer.atomConfig.webAudioBufferingTime, 40, 200);
					EditorGUI.indentLevel -= 1;
				}
			}
			EditorGUI.indentLevel -= 1;
