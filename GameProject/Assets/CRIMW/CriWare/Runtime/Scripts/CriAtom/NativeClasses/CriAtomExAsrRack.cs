...
public partial class CriAtomExAsrRack : CriDisposable
{
	...
	*/
	...
	/**
	 * <summary>デバイスIDの設定</summary>
	 * <param name='rackId'>ASRラックID</param>
	 * <param name='busName'>設定対象のバス名</param>
	 * <param name='deviceId'>デバイスID</param>
	 * <remarks>
	 * <para header='説明'>ASRラックから音声を出力する先のデバイスIDを設定します。</para>
	 * </remarks>
	 */
	public static void SetDeviceId_WASAPI(CriAtomExAsrRackId rackId, string busName, string deviceId) {
		...
		criAtomExAsrRack_SetDeviceId_WASAPI(rackId, busName, audioDevice);
	#endif
	}
	 ...
	 * <remarks>
	 * <para header='説明'>ASRラックから音声を出力する先のデバイスIDを設定します。<br/>
	 * 事前に CriAtomEx::LoadAudioDeviceList_PC を呼び出して
	 * プラグイン内でのデバイスリストを初期化してから、本関数を呼び出してください。</para>
	 * </remarks>
	 */
	public static void SetDeviceId_WASAPI(CriAtomExAsrRackId rackId, string busName, int index) {
	#if UNITY_EDITOR_WIN || UNITY_STANDALONE_WIN
		IntPtr id = CriAtomEx.GetAudioDeviceId_PC(index);
		criAtomExAsrRack_SetDeviceId_WASAPI(rackId, busName, id);
	#endif
	}
	 ...
	 * <param name='busName'>設定対象のバス名</param>
	 * <param name='index'>デバイスインデクス</param>
	 * <param name='deviceType'>ハプティクスデバイスタイプ</param>
	 * <remarks>
	 * <para header='説明'>ASRラックのハプティクスデバイスIDとそのタイプを指定します。<br/>
	 * 事前に CriAtomEx::LoadAudioDeviceList_PC を呼び出して
	 * プラグイン内でのデバイスリストを初期化してから、本関数を呼び出してください。</para>
	 * </remarks>
	 */
	public static void SetHapticDeviceId_PC(CriAtomExAsrRackId rackId, string busName, int index, CriAtom.HapticDeviceType_WASAPI deviceType) {
	#if UNITY_EDITOR_WIN || UNITY_STANDALONE_WIN
		IntPtr id = index < 0 ? IntPtr.Zero : CriAtomEx.GetAudioDeviceId_PC(index);
		criAtomExAsrRack_SetHapticDeviceId_WASAPI(rackId, busName, id, deviceType);
	#endif
	}
	...
	#if UNITY_EDITOR_WIN || UNITY_STANDALONE_WIN
	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern void criAtomExAsrRack_SetDeviceId_WASAPI(CriAtomExAsrRackId rack_id, string bus_name, [MarshalAs(UnmanagedType.LPWStr)] string device_id);
	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern void criAtomExAsrRack_SetDeviceId_WASAPI(CriAtomExAsrRackId rack_id, string bus_name, IntPtr device_id);
	#endif

	#if UNITY_EDITOR_WIN || UNITY_STANDALONE_WIN
	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern void criAtomExAsrRack_SetHapticDeviceId_WASAPI(int rack_id, string bus_name, IntPtr device_id, CriAtom.HapticDeviceType_WASAPI device_type);
	#endif
	...
	#if UNITY_EDITOR_WIN || UNITY_STANDALONE_WIN
	private static void criAtomExAsrRack_SetDeviceId_WASAPI(CriAtomExAsrRackId rack_id, string bus_name, [MarshalAs(UnmanagedType.LPWStr)]string  device_id) { }
	private static void criAtomExAsrRack_SetDeviceId_WASAPI(CriAtomExAsrRackId rack_id, string bus_name, IntPtr  device_id) { }
	#endif

	#if UNITY_EDITOR_WIN || UNITY_STANDALONE_WIN
	private static void criAtomExAsrRack_SetHapticDeviceId_WASAPI(int rack_id, string bus_name, IntPtr device_id, CriAtom.HapticDeviceType_WASAPI device_type) { }
	#endif
