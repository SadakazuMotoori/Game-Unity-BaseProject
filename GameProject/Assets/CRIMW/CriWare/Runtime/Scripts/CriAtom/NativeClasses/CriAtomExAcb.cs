...
public class CriAtomExAcb : CriDisposable
{
	...
	public static CriAtomExAcb LoadAcbData(byte[] acbData, CriFsBinder awbBinder, string awbPath)
	{
		...
		byte[] loadTarget = CriAtomPlugin.PrepareAcbDataForLoad(acbData);
		if (loadTarget == null) {
			return null;
		}

		IntPtr binderHandle = (awbBinder != null) ? awbBinder.nativeHandle : IntPtr.Zero;
		GCHandle gch = GCHandle.Alloc(loadTarget, GCHandleType.Pinned);
		if(string.IsNullOrEmpty(awbPath)){
			awbPath = null;
		}
		IntPtr handle = criAtomExAcb_LoadAcbData(
			gch.AddrOfPinnedObject(), loadTarget.Length, binderHandle, awbPath, IntPtr.Zero, 0);
		if (handle == IntPtr.Zero) {
			gch.Free();
			return null;
		}
	 ...
	 * マネージド領域のバイト配列を利用する場合はCriAtomExAcb.LoadAcbData(byte[], CriFsBinder, string)をご利用ください。<br/>
	 * <br/>
	 * インゲームプレビュー有効時に本メソッドを利用する場合、<br/>
	 * <see cref='CriWare.CriAtomExAcf.GetExtraWorkSizeForLoadAcbData()'/> で取得したサイズ分の空き領域をACBデータ末尾に追加した連続メモリ領域を確保し、<br/>
	 * <paramref name='dataSize'/> には「ACBデータサイズ + 追加した空き領域サイズ」の合計値を指定する必要があります。<br/></para>
	 * </remarks>
	 * <seealso cref='CriAtomExPlayer::SetCue'/>
	 */
...
public class CriAtomExAcbLoader : CriDisposable
{
	...
	public static CriAtomExAcbLoader LoadAcbDataAsync(byte[] acbData, CriFsBinder awbBinder, string awbPath, bool loadAwbOnMemory = false)
	{
		...
		IntPtr handle = criAtomExAcbLoader_Create(ref config);
		if (handle == IntPtr.Zero) { return null; }
		GCHandle dataHandle = GCHandle.Alloc(loadTarget, GCHandleType.Pinned);
		if(string.IsNullOrEmpty(awbPath)){
			awbPath = null;
		}
		bool result = criAtomExAcbLoader_LoadAcbDataAsync(handle, dataHandle.AddrOfPinnedObject(), loadTarget.Length, binderHandle, awbPath);
		if (result == false) {
			dataHandle.Free();
			criAtomExAcbLoader_Destroy(handle);
			return null; 
	 ...
	 * <para header='備考'>本メソッドはネイティブメモリ領域のデータを指定する際に利用することを想定しており、<br/>
	 * <paramref name='acbData'/>にはメモリ上で固定されたデータを示すポインタを指定する必要があります。<br/>
	 * マネージド領域のバイト配列を利用する場合はCriAtomExAcbLoader.LoadAcbDataAsync(byte[], CriFsBinder, string, bool)をご利用ください。<br/>
	 * <br/>
	 * インゲームプレビュー有効時に本メソッドを利用する場合、<br/>
	 * <see cref='CriWare.CriAtomExAcf.GetExtraWorkSizeForLoadAcbData()'/> で取得したサイズ分の空き領域をACBデータ末尾に追加した連続メモリ領域を確保し、<br/>
	 * <paramref name='dataSize'/> には「ACBデータサイズ + 追加した空き領域サイズ」の合計値を指定する必要があります。<br/></para>
