...
public class CriAtomExPlayer : CriDisposable
{
	...
	*/
	...
	/**
	 * <summary>3D音源再生位置更新タイプの設定</summary>
	 * <remarks>
	 * <para header='説明'>3Dポジショニング再生中に3D音源の位置が更新された場合の再生位置の振る舞いを設定します。<br/>
	 * 本関数で再生位置更新タイプを設定後、 CriWare.CriAtomExPlayer::Start 関数により再生開始すると、設定された再生位置更新タイプが適用されます。<br/>
	 * デフォルト値を設定する場合、ツール側で設定した再生位置の振る舞いが適用されます。<br/></para>
	 * <para header='備考'>本パラメーターは CriWare.CriAtomExPlayer::ResetParameters 関数にてクリアされます。<br/>
	 * 設定後、 CriWare.CriAtomExPlayer::Update 関数、 CriWare.CriAtomExPlayer::UpdateAll 関数を呼び出しても、
	 * すでに再生開始された波形データには適用されません。<br/>
	 * 3D音源ハンドルリストを用いた再生の場合、音源再生位置の振る舞いの設定は反映されず、再生位置は各3D音源の位置になります。<br/></para>
	 * </remarks>
	 * <seealso cref='CriAtomExPlayer::Update'/>
	 * <seealso cref='CriAtomExPlayer::UpdateAll'/>
	 * <seealso cref='CriAtomExPlayer::ResetParameters'/>
	 */
	public void Set3dSourcePlaybackPositionUpdateType(CriAtomEx.PlaybackPositionUpdateType type)
	{
		criAtomExPlayer_Set3dSourcePlaybackPositionUpdateType(this.handle, type);
	}
	...
	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern void criAtomExPlayer_Set3dSourcePlaybackPositionUpdateType(IntPtr player, CriAtomEx.PlaybackPositionUpdateType type);
#else
	...
	private static void criAtomExPlayer_Set3dSourcePlaybackPositionUpdateType(IntPtr player, CriAtomEx.PlaybackPositionUpdateType type) {}
#endif
    #endregion
}
