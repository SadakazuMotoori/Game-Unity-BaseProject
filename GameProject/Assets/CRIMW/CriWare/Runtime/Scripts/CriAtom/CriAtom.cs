...
public static class CriAtomPlugin
{
	...
	internal static bool UsesInGamePreview { get; private set; } = false;

	// インゲームプレビュー有効時のみ、ACB末尾に余白を足した連続バッファを作る。
	// ※無効時は元配列をそのまま返す。
	internal static byte[] PrepareAcbDataForLoad(byte[] acbData)
	{
		if (acbData == null)
		{
			return null;
		...
		if (!UsesInGamePreview)
		{
			return acbData;
		}

		int extraWorkSize = CriAtomExAcf.GetExtraWorkSizeForLoadAcbData();
		if (extraWorkSize <= 0)
		{
			return acbData;
		}

		byte[] expanded = new byte[acbData.Length + extraWorkSize];
		// 元のACBデータを拡張バッファの先頭にコピーし、末尾を追加ワーク領域として残す
		Buffer.BlockCopy(acbData, 0, expanded, 0, acbData.Length);
		return expanded;
	}

	public static byte[] PrepareAcfDataForRegister(byte[] acfData)
	{
		if (acfData == null)
		{
			return null;
		...
		int extraWorkSize = CriAtomEx.GetExtraWorkSizeForRegisterAcfData();
		if (extraWorkSize <= 0)
		{
			return acfData;
		}

		byte[] expanded = new byte[acfData.Length + extraWorkSize];
		Buffer.BlockCopy(acfData, 0, expanded, 0, acfData.Length);
		return expanded;
	}
	...
	public static void SetConfigParameters(int max_virtual_voices,
		...
		bool enable_atom_sound_disabled_mode, bool disable_aisac_graph_curve_type)
	{
		criAtomUnity_SetConfigParameters(max_virtual_voices,
			...
			enable_atom_sound_disabled_mode, disable_aisac_graph_curve_type
			);

		UsesInGamePreview = uses_in_game_preview;
	...
	public static void SetConfigAdditionalParameters_WEBGL(int num_webaudio_voices, bool use_webaudio_mixer, int webaudio_buffering_time)
	{
		#if UNITY_WEBGL
		criAtomUnity_SetConfigAdditionalParameters_WEBGL(num_webaudio_voices, use_webaudio_mixer, webaudio_buffering_time);
		#endif
	}
	...
	private static void RegisterForceFinalize(){
		UnityEditor.AssemblyReloadEvents.beforeAssemblyReload += ForceFinalize;
	}
	...
	#if UNITY_WEBGL
	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern void criAtomUnity_SetConfigAdditionalParameters_WEBGL(int num_webaudio_voices, bool use_webaudio_mixer, int webaudio_buffering_time);
	#endif
	...
	#if UNITY_WEBGL
	private static void criAtomUnity_SetConfigAdditionalParameters_WEBGL(int num_webaudio_voices, bool use_webaudio_mixer, int webaudio_buffering_time) { }
	#endif
...
public partial class CriAtom : CriMonoBehaviour
{
        ...
        /**
		 * <summary>ハプティクスデバイスタイプ</summary>
		 * <remarks>
		 * <para header='説明'>Windowsで使用するハプティクスデバイスのタイプです。<br/></para>
		 * </remarks>
		 */
        public enum HapticDeviceType_WASAPI : Int32
        {
            Default = 0,	/**< ステレオ出力を振動に使用 */
            Dls,			/**< Ls／Rs成分を振動に使用 */
        }
	...
	*/
	...
	private IEnumerator RegisterAcfAsync(string acfFile, string dspBusSetting)
	{
		...
		using (var req = new WWW(acfPath)) {
			yield return req;
			this.acfData = CriAtomPlugin.PrepareAcfDataForRegister(req.bytes);
		}
		/*
		...
		*/
		...
		CriAtomEx.RegisterAcf(this.acfRegisterGCHandle.AddrOfPinnedObject(), this.acfData.Length);
