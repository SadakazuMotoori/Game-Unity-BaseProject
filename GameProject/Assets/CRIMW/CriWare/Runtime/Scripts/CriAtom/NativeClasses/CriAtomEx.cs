 ...
 *
 ****************************************************************************/
 #if UNITY_EDITOR_WIN || UNITY_EDITOR_OSX || UNITY_STANDALONE_WIN || UNITY_STANDALONE_OSX || UNITY_IOS || UNITY_TVOS || UNITY_ANDROID || UNITY_PS4 || UNITY_PS5 || UNITY_GAMECORE_XBOXONE || UNITY_GAMECORE_SCARLETT || UNITY_SWITCH || UNITY_SWITCH2
	#define CRIWARE_SUPPORTS_CRIATOMEXMONITOR
#endif
...
public static class CriAtomEx
{
	 ...
	 */
	 public enum SoundRendererType {
		Default = 0, /**< 出力方式を制限しない */
		Native = 1, /**< プラットフォームライブラリへ出力 */
		Asr = 2, /**< Atom Sound Rendererへ出力 */
		Extended = 3,
		Spatial = 4, /**< 立体音響出力 */
		Hw1 = (0 << 16) | Native, /**< 機種固有定義1 */
		Hw2 = (1 << 16) | Native, /**< 機種固有定義2 */
		Hw3 = (2 << 16) | Native, /**< 機種固有定義3 */
		Hw4 = (3 << 16) | Native, /**< 機種固有定義4 */
		Haptic = (0 << 16) | Extended, /**< 振動 */
		Pseudo = (1 << 16) | Extended, /**< 無音 */
		SpatialChannels = (0 << 16) | Spatial, /**< Channel Based Audio */
		Ambisonics = (1 << 16) | Spatial, /**< Ambisonics */
		Passtrough = (2 << 16) | Spatial, /**< Non-Spatial */
		Object = (3 << 16) | Spatial, /**< Object Based Audio */
	}
	...
	/**
	 * <summary>3D音源再生位置更新タイプ</summary>
	 * <remarks>
	 * <para header='説明'>3Dポジショニング再生中に3D音源の位置が更新された場合の再生位置の振る舞いを設定するためのデータ型です。<br/>
	 * CriAtomExPlayer::Set3dSourcePlaybackPositionUpdateType 関数で利用します。<br/></para>
	 * <para header='備考'>本パラメータは波形データの再生開始時に適用されます。波形データの再生途中に変更された場合は反映されません。<br/></para>
	 * </remarks>
	 * <seealso cref='CriAtomExPlayer::Set3dSourcePlaybackPositionUpdateType'/>
	 */
	public enum PlaybackPositionUpdateType {
		/*
		* デフォルト値。
		* 再生位置更新タイプを設定しません。
		* ツール側で設定した値が適用されます。
		*/
		...
		/*
		* 音源位置に追従。
		* 再生位置が3D音源の位置更新を反映します。
		*/
		Follow = 0,
		/*
		* 再生時に固定。
		* 再生位置を再生開始時の3D音源位置に固定します。
		*/
		Fixed = 1,
	}
	...
	/**
	* <summary>3D音源の直方体形状パラメーター</summary>
	* <remarks>
	* <para header='説明'>本構造体は、3D音源に対して直方体形状を設定する際の
	* 各辺（幅・高さ・奥行き）の長さを指定するためのパラメーターです。<br/>
	* CriAtomEx3dSource::SetCuboidExtent 関数に渡して使用します。<br/></para>
	* <para header='備考'>すべての辺の長さが 0.0f の場合、直方体形状は無効化され、3D音源は点音源として扱われます。<br/></para>
	* <para header='注意'>各パラメーターには 0.0f 以上の数値を指定してください。<br/></para>
	* </remarks>
	* <seealso cref='CriAtomEx3dSource::SetCuboidExtent'/>
	*/
	[StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi)]
	public struct CuboidExtent {
		public float width;                /**< 横幅 */
		public float height;               /**< 高さ */
		public float depth;              /**< 奥行き */
		public static CuboidExtent Default(){
			return new CuboidExtent
			{
				width     = 0.0f,
				height    = 0.0f,
				depth   = 0.0f,
	...
	public struct CuePos3dInfo
	{
		...
		public float coneOutsideVolume;                          /**< コーン外部音量 */
		...
		public CuePos3dInfo(byte[] data, int startIndex)
		{
			...
			this.coneOutsideVolume = BitConverter.ToSingle(data, startIndex + 8);
			this.minAttenuationDistance = BitConverter.ToSingle(data, startIndex + 12);
			this.maxAttenuationDistance = BitConverter.ToSingle(data, startIndex + 16);
			this.sourceRadius = BitConverter.ToSingle(data, startIndex + 20);
			this.interiorDistance = BitConverter.ToSingle(data, startIndex + 24);
			this.dopplerFactor = BitConverter.ToSingle(data, startIndex + 28);
			this.randomPos = new Randomize3dConfig(data, startIndex + 32);
			this.distanceAisacControl = BitConverter.ToUInt32(data, startIndex + 52);
			this.listenerBaseAngleAisacControl = BitConverter.ToUInt32(data, startIndex + 56);
			this.sourceBaseAngleAisacControl = BitConverter.ToUInt32(data, startIndex + 60);
			this.listenerBaseElevationAisacControl = BitConverter.ToUInt32(data, startIndex + 64);
			this.sourceBaseElevationAisacControl = BitConverter.ToUInt32(data, startIndex + 68);
		}
	}
	...
	*/
	...
	public struct CueInfo
	{
		...
		public CueInfo(byte[] data, int startIndex)
		{
			if (IntPtr.Size == 4) {
				...
				this.volume    = BitConverter.ToSingle(data, startIndex + 156);
				this.silentMode = (SilentMode)BitConverter.ToInt32(data, startIndex + 160);
				this.pitch = BitConverter.ToSingle(data, startIndex + 164);
				this.selectorIndex = BitConverter.ToUInt16(data, startIndex + 168);
			} else {
				this.id = BitConverter.ToInt32(data, startIndex + 0);
				this.type   = (CueType)BitConverter.ToInt32(data, startIndex + 4);
	...
	/**
	 * <summary>ACFデータ登録時に必要な追加ワーク領域サイズの取得</summary>
	 * <returns>追加ワーク領域サイズ（byte単位）</returns>
	 * <remarks>
	 * <para header='説明'>インゲームプレビューに用いるACFデータをメモリ上から登録する際に、
	 * ACFデータの末尾に追加して確保する必要があるワーク領域サイズを取得します。<br/></para>
	 * <para header='説明'>CriWare.CriAtomEx::RegisterAcf(IntPtr, int) 関数にメモリ上のACFデータを渡す場合は、
	 * ACFデータ本体の直後に、本関数で取得したサイズ分の追加ワーク領域を含めたメモリ領域を確保してください。<br/></para>
	 * </remarks>
	 * <seealso cref='CriAtomEx::RegisterAcf'/>
	 */
	public static int GetExtraWorkSizeForRegisterAcfData()
	{
#if CRIWARE_SUPPORTS_CRIATOMEXMONITOR
		return criAtomEx_GetExtraWorkSizeForRegisterAcfData();
#else
		return 0;
#endif
	}
	 ...
	 * \deprecated
	 * 削除予定の非推奨APIです。
	 * CriWare.CriAtomEx.RegisterAcf(IntPtr, int)の使用を検討してください。
	*/
	 ...
	 * <remarks>
	 * <para header='説明'>Atomライブラリ内のマスタタイマを一時停止／再開します。<br/>
	 * マスタタイマを一時停止すると、シーケンス時刻が進行しなくなります。<br/></para>
	 * <para header='備考'>本関数は アプリケーションが休止したり一時停止するようなプラットフォームにおいて、<br/>
	 * 休止中や一時停止中でもタイマが進行してしまうプラットフォーム向けの機能です。<br/>
	 * アプリケーションが休止状態や一時停止状態に遷移する前に<br/>
	 * 本関数でマスタタイマを一時停止しておくことで、休止中のシーケンスの進行を止める事ができます。<br/></para>
	 * <para header='注意'>本関数で一時停止する対象はあくまでAtomライブラリ内のマスタタイマです。<br/>
	 * 音声の一時停止が必要な場合は<see cref='CriAtomExAsr.PauseOutputVoice'/>を利用してください。<br/></para>
	 * </remarks>
	 */
	...
	/**
	 * <summary>[PC] 仮想サイドスピーカー出力の有効化</summary>
	 * <param name='isEnable'>仮想サイドスピーカー出力を有効化するかどうか</param>
	 * <remarks>
	 * <para header='説明'>スペーシャルオーディオ機能を使用時、サイドスピーカーの代わりにリアスピーカーが存在する5.1ch構成において、
	 * サイドスピーカー出力をフロントスピーカーとリアスピーカーを用いて仮想的に表現します。
	 * 仮想サイドスピーカー出力を無効化する場合、サイドスピーカー出力は全てリアスピーカーから出力されます。</para>
	 * <para header='備考'>現行のライブラリでは、仮想サイドスピーカー出力はデフォルトで無効化されています。<br/>
	 * 本機能を有効化したい場合、 true を指定して実行してください。</para>
	 * </remarks>
	 */
	public static void SetVirtualSideSpeakerEnabled_PC(bool isEnable)
	{
#if UNITY_EDITOR_WIN || UNITY_STANDALONE_WIN
		criAtom_SetVirtualSideSpeakerEnabled_WASAPI(isEnable);
#endif
	}
	...
	/**
	 * <summary>[PC] 振動出力デバイスの設定(インデクス指定)</summary>
	 * <param name='index'>デバイスインデクス</param>
	 * <param name='device_type'>ハプティクスデバイスタイプ</param>
	 * <remarks>
	 * <para header='説明'>Atomからの振動出力先となるデバイスを設定します。<br/>
	 ...
	 */
	public static void SetOutputHapticDevice_PC(int index, CriAtom.HapticDeviceType_WASAPI device_type)
	{
#if UNITY_EDITOR_WIN || UNITY_STANDALONE_WIN
		IntPtr id = index < 0 ? IntPtr.Zero : criAtomUnity_GetAudioDeviceId_PC(index);
		criAtom_SetHapticDeviceId_WASAPI(id, device_type);
#endif
	}
	 ...
	 * <remarks>
	 * <para header='説明'>スペーシャルオーディオ機能（Microsoft Spatial Sound）を有効にします。<br/>
	 * 引数の type には、スペーシャルオーディオ機能を有効（又は無効）にするサウンドレンダラを指定します。<br/></para>
	 * <para header='備考'>現行のライブラリでは、スペーシャルオーディオ機能はデフォルトで有効です。<br/>
	 * そのため、スペーシャルオーディオ機能を無効にしたい場合を除き、本関数を明示的に呼び出す必要はありません。<br/></para>
	 * <para header='注意'>本関数は、CriWareInitializerのAwake実行前に本関数を呼び出す必要があります。<br/>
	 * また、SoundRendererTypeがNative（内部では「Hw1」と同じ値）以外の場合のみ、本関数を使用してください。<br/>
	 * SoundRendererTypeがNative/Hw1の出力先に対して設定を行う場合、本関数を使用せず、<br/>
	 * CriWareInitializerコンポーネント - PC Config - Microsoft Spatial Sound 項目で設定してください。<br/></para>
	 * </remarks>
	 */
	static public void SetSpatialAudioEnabled_WASAPI(CriAtomEx.SoundRendererType type, bool sw) {
#if UNITY_EDITOR_WIN || UNITY_STANDALONE_WIN
		criAtom_SetSpatialAudioEnabled_WASAPI(type, sw);
#endif
	}

	static internal IntPtr GetAudioDeviceId_PC(int index) {
#if UNITY_EDITOR_WIN || UNITY_STANDALONE_WIN
		return CriAtomEx.criAtomUnity_GetAudioDeviceId_PC(index);
#else
		return IntPtr.Zero;
#endif
	}
...
#if CRIWARE_SUPPORTS_CRIATOMEXMONITOR
	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern int criAtomEx_GetExtraWorkSizeForRegisterAcfData();
#endif
	...
	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern void criAtomEx_PauseTimer(Boolean sw);
	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	...
	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern void criAtom_SetVirtualSideSpeakerEnabled_WASAPI(bool is_enable);
	...
	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
    private static extern void criAtom_SetHapticDeviceId_WASAPI(IntPtr device_id, CriAtom.HapticDeviceType_WASAPI device_type);
	...
	[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
	private static extern void criAtom_SetSpatialAudioEnabled_WASAPI(CriAtomEx.SoundRendererType type, bool sw);
	#endif
...
#if CRIWARE_SUPPORTS_CRIATOMEXMONITOR		
	private static int criAtomEx_GetExtraWorkSizeForRegisterAcfData() { return 0; }
#endif
	...
	private static void criAtomEx_PauseTimer(Boolean sw) { }
	...
	#if UNITY_EDITOR_WIN || UNITY_STANDALONE_WIN
	private static void criAtom_SetVirtualSideSpeakerEnabled_WASAPI(bool is_enable) { }
	...
	private static void criAtom_SetHapticDeviceId_WASAPI(IntPtr device_id, CriAtom.HapticDeviceType_WASAPI device_type){ }
	...
	private static void criAtom_SetSpatialAudioEnabled_WASAPI(CriAtomEx.SoundRendererType type, bool sw){ }
	#endif
	#if !UNITY_EDITOR && UNITY_PSP2
...
public static class CriAtomExCategory
{
	 ...
	 * \deprecated
	 * 削除予定の非推奨APIです。
	 * CriWare.CriAtomEx.CurveType の使用を検討してください。
	*/
	...
	{
		/** <summary>REACT減衰ボリュームパラメータ構造体</summary> */
		[StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi)]
		public struct Volume
		{
            public float level;             /**< 減衰ボリュームレベル */
		}
		/** <summary>REACT AISACコントロール構造体</summary> */
		[StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi)]
		...
		/** <summary>REACT ターゲット構造体</summary> */
		[StructLayout(LayoutKind.Explicit, CharSet = CharSet.Ansi)]
        ...
        [FieldOffset(0)]
        public Target target;                       /**< ダッカーの操作対象 */
        [FieldOffset(8)]
        public ReactDuckerTargetType targetType;    /**< ダッカーの操作対象種類 */
        [FieldOffset(12)]
	...
	{
		/**
		 * <summary>REACT動作パラメータ構造体</summary>
		 * <remarks>
		 * <para header='説明'>REACTの動作パラメータ情報構造体です。<br/></para>
		 * </remarks>
		 */
		[StructLayout(LayoutKind.Explicit, CharSet = CharSet.Ansi)]
        ...
        [FieldOffset(0)]
        public Parameter parameter;         /**< REACT動作パラメータ */
        [FieldOffset(44)]
	 ...
	 * \deprecated
	 * 削除予定の非推奨APIです。
	 * CriWare.CriAtomExCategory.SetAisacControl の使用を検討してください。
	*/
...
public class CriAtomExAsr
{
	...
	*/
	 ...
	 * <para header='注意'>本APIはCRI ADX Audio Effect Plugin SDKでユーザ定義エフェクトを登録するときのみ使用可能です。<br/>
	 ...
	 * <para header='注意'>本APIはCRI ADX Audio Effect Plugin SDKでエフェクトの登録解除をするときのみ使用可能です。</para>
	 * </remarks>
	...
	/**
	 * <summary>すべての再生のポーズ／ポーズ解除</summary>
	 * <param name='sw'>true=音声一時停止 false=音声再開</param>
	 * <remarks>
	 * <para header='説明'>ADXライブラリ上のすべての再生をポーズ（一時停止）したり、ポーズを解除します。<br/>
	 * ポーズするか、ポーズを解除するかは、引数のflagで指定します。<br/>
	 * <br/>
	 * 本関数は主に以下の用途に利用します。<br/>
	 * 	- ゲーム全体の音声の一時停止</para>
	 * </remarks>
	 */
